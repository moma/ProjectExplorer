...
let SigmaUtils = function() {
    ...
    this.FillGraph = function( initialActivetypes, initialActivereltypes, catDict, nodes, edges, graph ) {
        ...
        for (let k in initialActivereltypes) {
          let reltype = initialActivereltypes[k];
          for (let srcnid in TW.Relations[reltype]) {
              for (let j in TW.Relations[reltype][srcnid]) {
...
return;
      ...
      let typeId = TW.catDict[node.type] || 0;
...
context.fillStyle = TW.gui.handpickedcolors[node.type].alton ? node.customAttrs.alt_color : node.color;
} // node's
        else {
context.fillStyle = '#fff';
        ...
        let nodeRenderer = sigma.canvas.nodes.def;
    ...
    this.twRender.canvas.edges.curve = function(edge, source, target, context, settings) {
      ...
      let defSize = edge[prefix + 'size'] || settings('minEdgeSize') || 1;

      // precomputed color with no opacity
      // cf. sigmaTools.edgeRGB
      let baseRGB = edge.customAttrs.useAltColor ? edge.customAttrs.alt_rgb : edge.customAttrs.rgb;
    ...
    this.twRender.canvas.edges.line = function(edge, source, target, context, settings) {
      ...
      let rgb = edge.customAttrs.rgb;
      let defSize = edge[prefix + 'size'] || 1;
      if (edge.customAttrs.activeEdge) {
        size = defSize * 1.5;
    ...
    this.twRender.canvas.nodes.withBorders = function(node, context, settings) {
        ...
        let borderSize = node[prefix + 'size'] + settings('twNodeRendBorderSize');
        let borderColor = settings('twNodeRendBorderColor') || '#000';
        let nodeSize = node[prefix + 'size'];
        let nodeColor = node.color || settings('defaultNodeColor');
...
return;
                ...
                let icon = createWaitIcon('layoutwait');
                let btn = document.querySelector('#layoutButton');
...
function find(lquery) {
    ...
    if (typeof lquery == 'string' && lquery.length > 0) {
        ...
        for (let i in TW.labels) {
            let labObj = TW.labels[i];
            if (labObj && labObj.label) {
                let possiblematch = labObj.label.toLowerCase();
                // ------------------
                //  substring search
                // ------------------
...
function gradientColoring(daclass, forTypes) {
 ...
 return nd.attributes[daclass];
    ...
    for (var k in forTypes) {
      ...
      for (var j in nids) {
        let nid = nids[j];
        let attval = getVal(TW.Nodes[nid]);

        let attnumber = Number(attval);
        if (isNaN(attnumber)) {
          continue;
        }

        let round_number = Math.round( attnumber*themult );
    ...
    let Min_size = 1;
    let Max_size= 8;
    let setSize = (TW.facetOptions[daclass] && TW.facetOptions[daclass].setsize);
    for (let nid in NodeID_Val) {
        var newval_color;
        ...
        let hex_color = rgbToHex(255, (255-newval_color), 0);
        ...
        if (n && !n.hidden) {
          ...
          if (setSize) {
            let newval_size = Math.round( ( Min_size+(NodeID_Val[nid]['round']-real_min)*((Max_size-Min_size)/(real_max-real_min)) ) );
            n.size = newval_size;
          }
...
function heatmapColoring(daclass, forTypes) {
  ...
  for (var k in forTypes) {
    ...
    // our binning
    let tickThresholds = TW.Facets[ty][daclass].invIdx;
    ...
    for (var k in tickThresholds) {
      ...
      if (tickThresholds[k].nids.length) {
        ...
        for (let j in tickThresholds[k].nids) {
          ...
          if (n) {
            ...
            let originalLabel = TW.Nodes[n.id].label;
            if (doModifyLabel) {
              let valSt = n.attributes[daclass];
...
function clusterColoring(daclass, forTypes) {
    ...
    if (daclass=='clust_default') {
        for (let nid in TW.Nodes) {
          let original_node_color = TW.Nodes[nid].color;
        ...
        // fallback on old, slower strategy if scanAttributes inactive
          ...
          for (let j in nids) {
            ...
            if (the_node) {
              ...
              if (! the_node.hidden) {
                let attval = ( !isUndef(the_node.attributes) && !isUndef(the_node.attributes[daclass]) )? the_node.attributes[daclass] : TW.partialGraph.graph.nodes(nid)[daclass];
