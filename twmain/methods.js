...
function readMenu(infofile) {
  ...
  let preRES = AjaxSync({url: infofile, datatype: 'json'});
  ...
  for ( let projectPath in preRES.data ) {
    if (! preRES.data[projectPath] || ! preRES.data[projectPath].length) {
      ...
      if (projectPath == 'first_project') {
        ...
        for (let l in preRES.data[projectPath]) {
...
console.info(`attempting to load project conf ${projectConfFile}`);
    ...
    let pjconfRes = AjaxSync({url: projectConfFile, datatype: 'json'});
      ...
      for (let ndtype in confEntry) {
        if (! /node\d+/.test(ndtype)) {
          ...
          if (confEntry[ndtype].reldbs) {
            ...
            for (let dbtype in confEntry[ndtype].reldbs) {
...
console.info(`attempting to load legends conf ${legendConfFile}`);
    ...
    let legconfRes = AjaxSync({url: legendConfFile, datatype: 'json'});
...
function getActivetypesNames() {
  ...
  for (let i = 0; i < TW.categories.length; i++) {
    if (activeFlags[i]) {
...
function getNActive(someState) {
  return TW.SystemState().activetypes.filter(function(bool) {
return bool;
}).length;
}
...
console.log('deselecting using SystemState\'s lists');
    ...
    for (let reltyp in rels) {
      for (let srcnid in rels[reltyp]) {
        for (let tgtnid in rels[reltyp][srcnid]) {
...
function redrawNodesInHoverLayer(someNodes, canvasDrawer) {
  ...
  let targetLayer = TW.rend.contexts.hover;
  ...
  let locSettings = TW.partialGraph.settings.embedObjects({prefix: TW.rend.options.prefix});

  for (let k in someNodes) {
...
function clearHover() {
  let hoverLayer = TW.rend.contexts.hover;
...
function updateRelatedNodesPanel( sels, same, oppos ) {
    ...
    if (sels.length>0) {
        ...
        let sameNeighTagcloudHtml = htmlProportionalLabels( same, TW.conf.tagcloudSameLimit, true );
...
function prepareEdgesRenderingProperties(edgesDict, nodesDict) {
  for (let eid in edgesDict) {
    let e = edgesDict[eid];

    e.weight = Math.round(e.weight*100000)/100000;
    // e.size = e.weight // REFA s/weight/size/ ?

    let rgbStr = sigmaTools.edgeRGB(nodesDict[e.source].color, nodesDict[e.target].color);
