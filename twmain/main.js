...
TW.SystemState = function() {
 return TW.states.slice(-1)[0];
};
...
if (window.location.protocol == 'file:' || TW.sourcemode == 'localfile') {
  ...
  let graphFileInput = createFilechooserEl();
...
// traditional cases: remote read from API or prepared server-side file
  ...
  let [inFormat, inData, mapLabel] = syncRemoteGraphData();
...
function syncRemoteGraphData() {
  let inFormat; // = { db|api.json , somefile.json|gexf }
  let inData; // = {nodes: [....], edges: [....], cats:...}

  let mapLabel; // user displayed label for this input dataset
  ...
  if (TW.sourcemode == 'api') {
    ...
    let sourceinfo = getUrlParam.srcparams;
    let qtype = getUrlParam.type;
    if (isUndef(sourceinfo) || isUndef(qtype)) {
        ...
        if (qtype == 'filter' || qtype == 'uid') {
          ...
          if (qtype=='filter') {
              ...
              let json_constraints = decodeURIComponent(sourceinfo);
              ...
              for (let fieldName in TW.APIQuery) {
                  // a nodetype
                  ...
                  // an array of filters
                    ...
                    let nameSubElts = [];
                    for (let value of TW.APIQuery[fieldName]) {
          ...
          if (! mapLabel) {
              ...
              for (var i in queryarray) {
                  ...
                  if (Array.isArray(item) && item.length>0) {
                      for (let j in item) elements.push(item[j]);
                  }
              }
              mapLabel = '"'+elements.join('" , "')+'"';
          }

          let bridgeRes = AjaxSync({url: theurl, data: thedata, type: 'GET'});
  ...
  // cases            (2)       and     (3) : read a file from server
    ...
    if (TW.sourcemode == 'servermenu') {
        ...
        if (! linkCheck(TW.conf.paths.sourceMenu)) {
          ...
          // chooser menu
          let files_selector = '<select onchange="openGraph(this.options[this.selectedIndex].dataset.fullpath)">';
          for (let projectPath in gmenu) {
    ...
    let finalRes = AjaxSync({url: TW.File});
...
function mainStartGraph(inFormat, inData, twInstance) {
  ...
  if (! inFormat || ! inData) {
    alert('error on data load');
      ...
      let initialActivetypes = twInstance.initialActivetypes( TW.categories );
      let initialActivereltypes = twInstance.inferActivereltypes( initialActivetypes );

      // XML parsing from ParseCustom into dicts of nodes by type or rels by etype
      let dicts = start.makeDicts(TW.categories); // > parse json or gexf, dictfy
      ...
      TW.graphData = sigmaUtils.FillGraph( initialActivetypes, initialActivereltypes, TW.catDict, TW.Nodes, TW.Edges, TW.graphData );
      ...
      if (!TW.conf.filterSliders) {
        let filterEls = document.getElementsByClassName('tworow-selectors');

        for (let k in filterEls) {
